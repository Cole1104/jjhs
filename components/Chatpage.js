...
export const ioC = io('https://d03c-39-113-114-105.ngrok-free.appp',{
    autoConnect: false,
...
export default function Chatpage(){
    ...
    useEffect(()=>{
       if(localStorage.getItem('isLogin') == 'false'){
        ...
        ioC.on('messaged',(value,name)=>{
            ...
            setChatLog(current=>[...current,{sender:"상대방",type:"user_message",message:value}])
        })
        ioC.on('byebye',()=>{
            ioC.disconnect();
            setMatchedRoom('')
    ...
    <div className=" fixed bottom-0 w-screen h-11 bg-slate-600">
        <form onSubmit={handleSubmit} className=" flex justify-center">
            <input type="textarea" ref={reff} className="h-7 m-2 rounded-md w-3/4"></input>
            <button className=" bg-slate-300 p-4 px-5  rounded-lg ">
            <svg class="w-4 h-3  text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
